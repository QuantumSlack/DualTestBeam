...
4. EdgeDetNoSegment:- Branch which contains the same information as the previous branch, but when particles are detected in the edge detector instead of detector's interior.
5. MCParticles (Monte-Carlo Particles):- This branch contains information about trackID, parents, daughters, pdgID, spin, genstatus etc. for each particle produced in the simulation irrespective of where they are.
6. Remaining parameters are dials which are specified in SCEPCALsteering.py file.
